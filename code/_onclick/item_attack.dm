...
/obj/item/proc/get_clamped_volume()
	if(w_class)
		if(force)
			return clamp((force * 2 + w_class) * 4, 30, 75)// Add the item's force to its weight class and multiply by 4, then clamp the value between 30 and 100
		else
			return clamp(w_class * 6, 10, 75) // Multiply the item's weight class by 6, then clamp the value between 10 and 100

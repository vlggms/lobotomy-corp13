...
/**
 * This comment is made for the sake of my sanity, its to make porting over TG's menu easier on myself.
 * It will get deleted on the PR being fully merged, dont worry
 *
 * CRITICAL DIFFERENCES:
 *
 * to_chat(new_player, span_notice("Step NT... what are you doing?"))
 * =
 * to_chat(new_player, "<span class='notice'>Step NT... what are you doing?</span>")
 *
 * update_appearance(UPDATE_ICON) / update_appearance(UPDATE_OVERLAYS)
 * =
 * update_icon() / update_overlays()
 *
 * Triggers:
 *
 * to_chat(new_player, span
 * update_app
 *
 *
 * number of problems fixed caused by porting: 22
 *
 */
...
/datum/hud/new_player/New(mob/owner)
	...
	for(var/button in buttons)
		var/atom/movable/screen/lobby/lobbyscreen = new button(our_hud = src)
...
//			lobbyscreen.RegisterSignal(src, COMSIG_HUD_LOBBY_EXPANDED, TYPE_PROC_REF(/atom/movable/screen/lobby, expand_button))
			///MODIFIED TG CODE (this may REALLY break something or cause a memory leak however they are caused, im not a coder so i have no clue)
			lobbyscreen.RegisterSignal(src, COMSIG_HUD_LOBBY_COLLAPSED, /atom/movable/screen/lobby.proc/collapse_button)
			lobbyscreen.RegisterSignal(src, COMSIG_HUD_LOBBY_EXPANDED, /atom/movable/screen/lobby.proc/expand_button)
...
/* This is here for a future settings menu that will probably never come because i cant be bothered to port it, and i dont like it :)... but im keeping the code for now

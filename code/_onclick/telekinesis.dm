...
/obj/item/tk_grab/proc/check_if_focusable(obj/target)
	if(!tk_user || !istype(tk_user) || QDELETED(target) || !istype(target) || (!tk_user.dna.check_mutation(TK) && !tk_user.mind.has_martialart(MARTIALART_STARTERSITH) ) ) // Tegustation Chaplain Traitor
		qdel(src)
		return

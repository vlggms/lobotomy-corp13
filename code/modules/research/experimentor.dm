...
/obj/machinery/rnd/experimentor/proc/experiment(exp,obj/item/exp_on)
	...
	if(exp == SCANTYPE_POKE)
		...
		if(prob(EFFECT_PROB_LOW) && criticalReaction)
			...
			for(var/mob/living/m in oview(1, src))
				m.deal_damage(15, BRUTE, flags = (DAMAGE_NO_SPREAD), def_zone = pick(BODY_ZONE_HEAD,BODY_ZONE_CHEST,BODY_ZONE_PRECISE_GROIN))
	...
	if(exp == SCANTYPE_HEAT)
		...
		if(prob(EFFECT_PROB_LOW) && criticalReaction)
			...
			throwSmoke(loc)
			for(var/mob/living/m in oview(1, src))
				m.deal_damage(5, FIRE, flags = (DAMAGE_NO_SPREAD), def_zone = pick(BODY_ZONE_HEAD,BODY_ZONE_CHEST,BODY_ZONE_PRECISE_GROIN))
	...
	if(prob(EFFECT_PROB_VERYLOW-badThingCoeff) && loaded_item)
		...
		if(globalMalf > 36 && globalMalf < 50)
			...
			for(var/mob/living/m in view(4,src))
				...
				m.deal_damage(50, BRUTE, def_zone = BODY_ZONE_CHEST)

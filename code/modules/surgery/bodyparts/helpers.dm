...
/mob/living/carbon/proc/newBodyPart(zone, robotic, fixed_icon)
	...
	switch(zone)
		if(BODY_ZONE_L_ARM)
			L = new part_default_l_arm() // TEGU // Why can't individual humans get different parts? // /obj/item/bodypart/l_arm()
		if(BODY_ZONE_R_ARM)
			L = new part_default_r_arm() // TEGU /  //obj/item/bodypart/r_arm()
		if(BODY_ZONE_HEAD)
			L = new part_default_head()  // TEGU /  //obj/item/bodypart/head()
		if(BODY_ZONE_L_LEG)
			L = new part_default_l_leg() // TEGU /  //obj/item/bodypart/l_leg()
		if(BODY_ZONE_R_LEG)
			L = new part_default_r_leg() // TEGU /  //obj/item/bodypart/r_leg()
		if(BODY_ZONE_CHEST)
			L = new part_default_chest() // TEGU /  //obj/item/bodypart/chest()
	if(L)

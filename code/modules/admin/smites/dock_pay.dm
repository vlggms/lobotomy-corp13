...
/datum/smite/dock_pay/effect(client/user, mob/living/target)
	...
	if (card.registered_account.account_balance == 0)
		...
		return
	var/new_cost = input("How much pay are we docking? Current balance: [card.registered_account.account_balance] ahn.", "BUDGET CUTS") as num|null
	if (!new_cost)
		return
	if (!(card.registered_account.has_money(new_cost)))
		to_chat(user,  "<span class='warning'>ID Card lacked funds. Emptying account.</span>")
		card.registered_account.bank_card_talk("[new_cost] ahn deducted from your account based on performance review.")

	...
	if(in_range(fatty, src))
		for(var/mob/living/L in get_turf(fatty))
			...
			if(istype(C))
				...
				if(prob(30))
					C.deal_damage(max(0, squish_damage - crit_rebate), flags = (DAMAGE_FORCED)) // the 30% chance to spread the damage means you escape breaking any bone
				else
				...
				"<span class='userdanger'>You are crushed by [src]!</span>")
				L.deal_damage(squish_damage, flags = (DAMAGE_FORCED))
				if(crit_case)
					L.deal_damage(squish_damage, flags = (DAMAGE_FORCED))

...
/////////////HARDHEAD MACHINIST HARDSUIT///////////////

/obj/item/clothing/head/helmet/space/hardsuit/ancient/hardhead
	name = "equipment operator helmet"
	desc = "Early prototype RIG hardsuit helmet, designed to quickly shift over a user's head. Designed to protect its user from operating unshielded corporate tech."
	icon_state = "hardsuit0-ancient"
	inhand_icon_state = "anc_helm"
...
/obj/item/clothing/suit/space/hardsuit/ancient/hardhead
	name = "equipment operator hardsuit"
	desc = "Prototype RIG hardsuit utilized by equipment operators from Hardhead, for safety in their workshop while operating what remains of old corporate technology."
	icon_state = "hardsuit-ancient"
	inhand_icon_state = "anc_hardsuit"
	armor = list(RED_DAMAGE = 50, WHITE_DAMAGE = 40, BLACK_DAMAGE = 50, PALE_DAMAGE = 40)
	slowdown = 3
	helmettype = /obj/item/clothing/head/helmet/space/hardsuit/ancient/hardhead
	resistance_flags = FIRE_PROOF
	var/footstep = 1
	var/mob/listeningTo

/obj/item/clothing/suit/space/hardsuit/ancient/hardhead/proc/on_mob_move()
	var/mob/living/carbon/human/H = loc
	if(!istype(H) || H.wear_suit != src)
		return
	if(footstep > 1)
...
/obj/item/clothing/suit/space/hardsuit/ancient/hardhead/equipped(mob/user, slot)
	. = ..()
	if(slot != ITEM_SLOT_OCLOTHING)
		if(listeningTo)
			UnregisterSignal(listeningTo, COMSIG_MOVABLE_MOVED)
...
/obj/item/clothing/suit/space/hardsuit/ancient/hardhead/dropped()
	. = ..()
	if(listeningTo)
		UnregisterSignal(listeningTo, COMSIG_MOVABLE_MOVED)

/obj/item/clothing/suit/space/hardsuit/ancient/hardhead/Destroy()
	listeningTo = null
	return ..()

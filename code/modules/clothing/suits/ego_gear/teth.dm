/obj/item/clothing/suit/armor/ego_gear/training
	name = "standard training armor"
	desc = "E.G.O intended for Manager Education"
	icon_state = "training"
	armor = list(RED_DAMAGE = 60, WHITE_DAMAGE = 10, BLACK_DAMAGE = -40, PALE_DAMAGE = 0)

/obj/item/clothing/suit/armor/ego_gear/beak
	name = "beak"
	desc = "It's sleek without a dent. The suit was originally very small, barely wearable by a child."
	icon_state = "beak"
	armor = list(RED_DAMAGE = 40, WHITE_DAMAGE = 30, BLACK_DAMAGE = -10, PALE_DAMAGE = 0)

/obj/item/clothing/suit/armor/ego_gear/match
	name = "fourth match flame"
	desc = "The archetype was already charred from the moment of extraction. \
	Although the exterior is scorched, it has no adverse effects on the E.G.O's performance."
	icon_state = "match"
	armor = list(RED_DAMAGE = 50, WHITE_DAMAGE = 10, BLACK_DAMAGE = -10, PALE_DAMAGE = 0)

/obj/item/clothing/suit/armor/ego_gear/fragment
	name = "fragments from somewhere"
	desc = "Do not attempt to understand it, just use it. \
	Sometimes, the wielder may see things we have long forgotten."
	icon_state = "fragment"
	armor = list(RED_DAMAGE = 10, WHITE_DAMAGE = -10, BLACK_DAMAGE = 50, PALE_DAMAGE = 0)

...
/obj/item/clothing/head/helmet/monkey_sentience/equipped(mob/user, slot)
	...
	if(!ismonkey(user) || user.ckey)
		...
		something.deal_damage(5,BRUTE, flags = (DAMAGE_NO_SPREAD), def_zone = BODY_ZONE_HEAD) //notably: no damage resist (it's in your helmet), no damage spread (it's in your helmet)
	...
	if(!polling)//put on a viable head, but taken off after polling finished.
		...
		if(prob(10))
			switch(rand(1,4))
				...
				if(2) //brain death
					magnification.deal_damage(500,BRAIN, flags = (DAMAGE_NO_SPREAD), def_zone = BODY_ZONE_HEAD)

...
/obj/item/firing_pin/paywall/gun_insert(mob/living/user, obj/item/gun/G)
	...
	gun.pin = src
	if(multi_payment)
		gun.desc += "<span class='notice'> This [gun.name] has a per-shot cost of [payment_amount] ahn[( payment_amount > 1 ) ? "s" : ""].</span>"
		return
	gun.desc += "<span class='notice'> This [gun.name] has a license permit cost of [payment_amount] ahn[( payment_amount > 1 ) ? "s" : ""].</span>"
	return
...
/obj/item/firing_pin/paywall/pin_auth(mob/living/user)
	...
	if(credit_card_details && !active_prompt)
		var/license_request = alert(user, "Do you wish to pay [payment_amount] ahn[( payment_amount > 1 ) ? "s" : ""] for [( multi_payment ) ? "each shot of [gun.name]" : "usage license of [gun.name]"]?", "Weapon Purchase", "Yes", "No")
		active_prompt = TRUE

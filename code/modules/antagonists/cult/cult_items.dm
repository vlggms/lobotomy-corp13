...
/obj/item/melee/cultblade/attack(mob/living/target, mob/living/carbon/human/user)
	if(!iscultist(user))
		...
		if(ishuman(user))
			...
			H.deal_damage(rand(force/2, force), BRUTE, def_zone = pick(BODY_ZONE_L_ARM, BODY_ZONE_R_ARM))
		else
	...
	if(!iscultist(user))
		...
		if(iscarbon(user))
			...
			if(C.active_hand_index == 1)
				C.deal_damage(20, BRUTE, def_zone = BODY_ZONE_L_ARM, wound_bonus = 20, sharpness = SHARP_EDGED) //oof ouch
			else
				C.deal_damage(20, BRUTE, def_zone = BODY_ZONE_R_ARM, wound_bonus = 20, sharpness = SHARP_EDGED)
		qdel(src)
		return FALSE

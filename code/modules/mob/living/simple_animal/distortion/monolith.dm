...
/obj/machinery/monolith/proc/Convert(mob/living/current_target)
	if(ishuman(current_target) && convert_humans)
		current_target.BecomeDistortion()
		LoseTarget()
		return TRUE
	var/mob/living/simple_animal/hostile/distortion/D = current_target
	if(istype(D, /mob/living/simple_animal/hostile/distortion))
		if(D.monolith_abnormality)
			var/mob/living/simple_animal/hostile/abnormality/myAbno = D.monolith_abnormality
			myAbno = new(get_turf(current_target))
			myAbno.BreachEffect()
			qdel(current_target)
			LoseTarget()
			return TRUE
...
/obj/machinery/monolith/admin/attack_hand(mob/living/user)
	if(ishuman(user))
		current_target = user
		Fire(current_target)
...
/mob/living/simple_animal/hostile/abnormality/forsaken_murderer
	...
	/* ego_list is the things you can buy with its unique type of PE.
		...
		here are datums that contain the cost and typepath of the
		accosiated object. You can find the files of these in: _ego_datum.dm
		The weapons and armor themselves are in _ego_gear.dm , ego_gun file ,
		and _ego_weapon.dm */
	ego_list = list(
		/datum/ego_datum/weapon/regret,
		/datum/ego_datum/armor/regret
		)
	/*Special employees get from interacting with the abnormality.
...
/mob/living/simple_animal/hostile/abnormality/forsaken_murderer/FailureEffect(mob/living/carbon/human/user, work_type, pe)
	/*Each abnormality has a variable called datum_refrence.
		The Qliphoth Counter, the thing that keeps abnormalities
		contained, is defined in this datum so we have to refrence
...
/* This is #define, it functions like a macro, paraphrasing repeating code.
	...
	There isnt much point placing it here instead of at the top of the
	file but i want to make a example of how to use it.
	The phrase placed after define is the word that is always
	replaced with the code after it. Im going to use this to shorten the cooldown change.
	Please check the bottom of the file to see #undef where you undefine the word so that
	it isnt replaced every time its written in the codebase.*/
...
/mob/living/simple_animal/hostile/abnormality/forsaken_murderer/Worktick(mob/living/carbon/human/user)
	...
	/*Sort of self explanatory, this is if the cooldown variable is less than the current time in the world.
		prob is a percent chance probability of 50. I placed the 50% chance here because i want it to be sort
		of unpredictable. */
	if(!client && work_emote_cooldown <= world.time && prob(35))
		/*Roll is a proc from the offical Dream Maker Guide. 1 is the amount of dice and 6 is the sides of those dice.
			I tried to put the link to the guide here but the codebase got upset so just try to find it at the
			byond website. */
		switch(roll(1, 6))
		//Switch only allows one of the ifs within it to proc, this is helpful with sorting number effects.
			...
			if(6)
				...
				/*Playsound is a unique proc that... plays a sound at the defined location. The first slot is location,
				second spot is the sound that is played, third is the volume of the sound, four is if it has a random pitch, \
				five is sound range. So i guess something can be quiet but heard normally if its sound range is high? */
...
/mob/living/simple_animal/hostile/abnormality/forsaken_murderer/BreachEffect(mob/living/carbon/human/user) //causes breach?
	...
	/*When i first heard about sleep being a proc that delays code progression until
		it is done it was said in hushed tones as if it was a taboo to use it.
		Also i set it to 13 because there is 12 frames in the flick icon and i
		wanted a additional 0.1 second for it to show. Just so you know most
		things without SECONDS or MINUTES after it are in deciseconds*/
	sleep(13)
	/*This made forsaken murderer waddle. Im supprised it was this easy to do this.
		Elements and Components are special non object code that you can attach to things.
		Element Variables are shared amongst all instances of it but component are individual
		and able to be altered without changing all the other instances of it.*/
	AddElement(/datum/element/waddling)
	/* 1 is distance thrown, False is if it can throw anchored objects, True if doesnt apply
		damage or stun when hits a wall. */

...
/mob/living/simple_animal/hostile/abnormality/cinderella/update_icon_state()
	icon_state = "cinderella_[max(1, round(freshness / 3))]"
...
/mob/living/simple_animal/hostile/abnormality/cinderella/ZeroQliphoth(mob/living/carbon/human/user)
	...
	addtimer(CALLBACK(GLOBAL_PROC, .proc/sound_to_playing_players, 'sound/abnormalities/cinderella/bell.ogg', 25), 30 SECONDS)
	addtimer(CALLBACK(src, .proc/death), 45 SECONDS)
...
/mob/living/simple_animal/hostile/abnormality/cinderella/proc/FireCarriage(aimpoint, direction = pick(EAST, WEST), aimZ = src.z)
	...
	for(var/i = 0, i < maxSegments * 2, i++)
		if(!(i % 2)) // True whenever i is even- this is the start of each segment
			persX += xIncrement*0.60 // Makes persX (effectively) smaller for this iteration
		else
			persX += xIncrement
		spawnPoint = locate(spawnX + persX, aimpoint, aimZ)
		var/obj/effect/cinderella/seg = new(spawnPoint)
		seg.dir = direction
		seg.icon_state = "cinderella_[i % 2 + 1]"
		if(i % 2)
			persX -= xIncrement/4
		if(i == 0)
			notify_ghosts("[seg] is preparing to depart!", source = seg, action = NOTIFY_ORBIT, header="Something Interesting!") // bless this mess
		segments += seg
	addtimer(CALLBACK(GLOBAL_PROC, .proc/sound_to_playing_players, 'sound/abnormalities/cinderella/announce.ogg', 50), 1)
	addtimer(CALLBACK(src, .proc/MoveCarriage), 10 SECONDS)

/mob/living/simple_animal/hostile/abnormality/cinderella/proc/MoveCarriage()
	if(LAZYLEN(src.segments))
		addtimer(CALLBACK(src, .proc/MoveCarriage), 0.5)
		...
		for(var/turf/T in coveredTurfs)
			for(var/mob/living/M in T.contents)
				if(M in src.damaged)
					continue
				src.damaged += M
				if(!seg.noise && seg.icon_state == "cinderella_1")
					if(rand())

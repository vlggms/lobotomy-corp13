...
/mob/living/simple_animal/hostile/abnormality/melting_love/CanAttack(atom/the_target)
	if(isliving(the_target) && !ishuman(the_target))
		...
		if(L.stat == DEAD)
			return FALSE
		if(L.type == /mob/living/simple_animal/hostile/abnominion/slime && health <= maxHealth * 0.8) // We need healing
			return TRUE
	return ..()
...
/mob/living/simple_animal/hostile/abnormality/melting_love/AttackingTarget(atom/attacked_target)
	...
	if(attacked_target.type == /mob/living/simple_animal/hostile/abnominion/slime)
		var/mob/living/simple_animal/hostile/abnominion/slime/S = attacked_target
...
/mob/living/simple_animal/hostile/abnormality/melting_love/proc/SlimeConvert(mob/living/carbon/human/H)
	...
	new /mob/living/simple_animal/hostile/abnominion/slime(get_turf(H))
...
/mob/living/simple_animal/hostile/abnormality/melting_love/proc/SpawnBigSlime(mob/living/simple_animal/hostile/abnominion/slime/big/S)
	gifted_human = null
...
/mob/living/simple_animal/hostile/abnominion/slime/Login()
	. = ..()
...
/mob/living/simple_animal/hostile/abnominion/slime/Initialize()
	. = ..()
...
/mob/living/simple_animal/hostile/abnominion/slime/proc/decay()
	to_chat(src, span_userdanger("You feel yourself falling apart..."))
	src.deal_damage(decay_damage, BLACK_DAMAGE)
	if (stat != DEAD)
		addtimer(CALLBACK(src, PROC_REF(decay)), decay_timer SECONDS, TIMER_STOPPABLE)

/mob/living/simple_animal/hostile/abnominion/slime/death()
	...
	if(SSmaptype.maptype == "rcorp")
		...
		for(var/mob/living/L in view(death_slime_range, src))
			if(L.stat != DEAD && !istype(L, /mob/living/simple_animal/hostile/abnominion/slime))
				L.apply_damage(death_damage, BLACK_DAMAGE, null, L.run_armor_check(null, BLACK_DAMAGE))
	return ..()

/mob/living/simple_animal/hostile/abnominion/slime/CanAttack(atom/the_target)
	if(isliving(the_target) && !ishuman(the_target))
		var/mob/living/L = the_target
		if(L.stat == DEAD)
			return FALSE
...
/mob/living/simple_animal/hostile/abnominion/slime/AttackingTarget(atom/attacked_target)
	// Convert
	if(ishuman(attacked_target))
		var/mob/living/carbon/human/H = attacked_target
		if(H.stat == DEAD || H.health <= HEALTH_THRESHOLD_DEAD)
...
/mob/living/simple_animal/hostile/abnominion/slime/proc/SlimeConvert(mob/living/carbon/human/H)
	if(!istype(H))
		return FALSE
	visible_message(span_danger("[src] glomps on \the [H] as another slime pawn appears!"))
	new /mob/living/simple_animal/hostile/abnominion/slime(get_turf(H))
...
/mob/living/simple_animal/hostile/abnominion/slime/proc/NestedItems(mob/living/simple_animal/hostile/nest, obj/item/nested_item)
	if(nested_item)
...
/datum/status_effect/display/melting_love_blessing/proc/Dissolve(mob/living/carbon/human/H)
	if(H)
		...
		if(H.stat == DEAD)
			var/mob/living/simple_animal/hostile/abnominion/slime/big/new_mob = new(owner.loc)

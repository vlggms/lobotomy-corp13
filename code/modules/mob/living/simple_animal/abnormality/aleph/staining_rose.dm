...
/mob/living/simple_animal/hostile/abnormality/staining_rose/WorktickFailure(mob/living/carbon/human/user)
	...
	user.deal_damage(rand(work_damage_lower,work_damage_upper), work_damage_type, source = src, flags = (DAMAGE_FORCED), attack_type = (ATTACK_TYPE_OTHER))
	WorkDamageEffect()
	if(user != chosen)
		return
	Stain(-2)
...
/mob/living/simple_animal/hostile/abnormality/staining_rose/proc/Bloom(working = FALSE)
	...
	if(!chosen)
		...
		chosen.deal_damage(damagetodo, PALE_DAMAGE, source = src, flags = (DAMAGE_FORCED), attack_type = (ATTACK_TYPE_OTHER))
	...
	if(qliphoth_lost) // Even more punishing if it was ignored.
		if(chosen)
			...
			for(var/i = numbermarked, i>=1, i--)
				...
				Y.deal_damage(25, PALE_DAMAGE, source = src, flags = (DAMAGE_FORCED), attack_type = (ATTACK_TYPE_OTHER))
			worse_breach = FALSE
...
/datum/status_effect/stained/proc/DealDamage()
	...
	switch(other_works_count)
		...
		if(4 to INFINITY)
			...
			breachtime = TRUE
	owner.deal_damage(damage_dealt, BRUTE, source = src, flags = (DAMAGE_FORCED), attack_type = (ATTACK_TYPE_OTHER))

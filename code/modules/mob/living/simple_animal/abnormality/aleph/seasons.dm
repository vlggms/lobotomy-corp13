...
/mob/living/simple_animal/hostile/abnormality/seasons/proc/SummerMelee(atom/attacked_target)
	...
	for(var/i = 1 to 20)
		...
		line = getline(T, T2)
		addtimer(CALLBACK(src, PROC_REF(DoLineAttack), line, TT, src), i * 0.12)
...
/obj/item/ego_weapon/sunspit/proc/DoLineAttack(list/line, atom/target, mob/living/carbon/human/user)
	...
	for(var/turf/T in line)
		...
		been_hit = HurtInTurf(T, been_hit, aoe_damage, BURN, check_faction = TRUE)
...
/mob/living/simple_animal/hostile/abnormality/seasons/proc/WinterMelee(atom/attacked_target)
	...
	for(var/i = 1 to projectile_number)
		...
		if(T.density)
			i -= 1
			continue
		var/obj/projectile/season_projectile/winter/P
		P = new(T)
		P.starting = T
		P.firer = src
		P.fired_from = T
		...
		P.color = COLOR_DARK_CYAN
		addtimer(CALLBACK (P, TYPE_PROC_REF(/obj/projectile, fire)), 10)
		var/list/hit_line = getline(T, get_turf(target)) //targetting line
		for(var/turf/TF in hit_line)
			if(TF.density)
				break
			var/obj/effect/temp_visual/cult/sparks/thespark = new(TF)
			thespark.color = COLOR_MODERATE_BLUE

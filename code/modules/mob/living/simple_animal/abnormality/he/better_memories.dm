...
/mob/living/simple_animal/hostile/abnormality/better_memories/proc/SpawnMinion(turf/spawn_turf)
	var/mob/living/simple_animal/hostile/abnominion/better_memories_minion/spawningmonster = new(spawn_turf)
...
//Minion Spawn
/mob/living/simple_animal/hostile/abnominion/better_memories_minion
	name = "Memories from a Better Time"
...
/mob/living/simple_animal/hostile/abnominion/better_memories_minion/Move()
	if(!can_act)
		return FALSE
	return ..()

//The creature can walk over entities that are human sized or smaller.
/mob/living/simple_animal/hostile/abnominion/better_memories_minion/CanPassThrough(atom/blocker, turf/target, blocker_opinion)
	if(isliving(blocker))
...
/mob/living/simple_animal/hostile/abnominion/better_memories_minion/OpenFire()
	if(!can_act)
		return FALSE
...
* to you.
*/
/mob/living/simple_animal/hostile/abnominion/better_memories_minion/CanAttack(atom/the_target)
	. = ..()
...
/mob/living/simple_animal/hostile/abnominion/better_memories_minion/AttackingTarget(atom/attacked_target)
	if(!can_act)
		return FALSE
	if(!client)
		if(!target)
...
/mob/living/simple_animal/hostile/abnominion/better_memories_minion/MoveToTarget(list/possible_targets)
	. = ..()
...
/mob/living/simple_animal/hostile/abnominion/better_memories_minion/patrol_select()
...
/mob/living/simple_animal/hostile/abnominion/better_memories_minion/patrol_reset()
	. = ..()
	FindTarget()
	if(current_target)
...
* -IP
*/
/mob/living/simple_animal/hostile/abnominion/better_memories_minion/bullet_act(obj/projectile/P)
	if(fleeing_now == TRUE)
		if(prob(50))
...
/mob/living/simple_animal/hostile/abnominion/better_memories_minion/attacked_by(obj/item/I, mob/living/L)
	//Stolen MOSB code.
...
/mob/living/simple_animal/hostile/abnominion/better_memories_minion/CanStartPatrol()
	if(!fleeing_now)
		return !(status_flags & GODMODE)
	return ..()

//Prevents accumulation of hate when actively fleeing.
/mob/living/simple_animal/hostile/abnominion/better_memories_minion/RegisterAggroValue(atom/remembered_target, value, damage_type)
...
/mob/living/simple_animal/hostile/abnominion/better_memories_minion/proc/FleeDest()
...
*/
/mob/living/simple_animal/hostile/abnominion/better_memories_minion/proc/FleeNow(turf/target_dest)
	...
	if(patrol_to(FleeDest()))
		addtimer(CALLBACK(src, TYPE_PROC_REF(/mob/living/simple_animal/hostile/abnominion/better_memories_minion, StopFleeing)), 1.5 SECONDS)
		return
	StopFleeing()

/mob/living/simple_animal/hostile/abnominion/better_memories_minion/proc/StopFleeing()
	fleeing_now = FALSE
	toggle_ai(AI_ON)

//This proc is for preventing the camera from firing at a target that is in its blind spot.
/mob/living/simple_animal/hostile/abnominion/better_memories_minion/proc/AngleCamera(atom/cam_focus)
...
/mob/living/simple_animal/hostile/abnominion/better_memories_minion/proc/CameraFlash(mob/living/user)
	var/blind_direction
	if(client)
...
/mob/living/simple_animal/hostile/abnominion/better_memories_minion/proc/ConeHelper(turf/starter_turf, dir_to_use, cone_levels = CAMERAFLASH_RANGE)
...
/mob/living/simple_animal/hostile/abnominion/better_memories_minion/proc/DoConeEffects(list/target_turf_list, mob/user, telegraph)
...
/mob/living/simple_animal/hostile/abnominion/better_memories_minion/proc/DoConeTurfEffects(turf/target_turf, type)
	if(type == 1)
...
/mob/living/simple_animal/hostile/abnominion/better_memories_minion/proc/DoConeMobEffect(mob/living/target_mob)
...
/mob/living/simple_animal/hostile/abnominion/better_memories_minion/proc/CalculateConeShape(current_level)
...
/mob/living/simple_animal/hostile/abnominion/better_memories_minion/proc/FindWorking()

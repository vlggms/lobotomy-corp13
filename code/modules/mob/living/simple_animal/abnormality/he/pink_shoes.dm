...
/mob/living/simple_animal/hostile/abnormality/pink_shoes/proc/AssimilateSimple(mob/living/simple_animal/hostile/user)
	...
	if(!special_appearance)//TODO: fix this shit
		var/atom/movable/DICKS = new /obj/effect/DICKS
		...
		DICKS.layer = (user.layer + 0.1)
		DICKS.add_filter("mob_shape_mask", 1, alpha_mask_filter(icon = mob_mask))
		user.vis_contents += DICKS
	user.SpeedChange(-1)
	user.UpdateSpeed()
	user.maxHealth += 1500

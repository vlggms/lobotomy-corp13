	...
	if(!possessee)//normal breach
		...
		for(var/turf/open/T3 in view(2, T))
			if(!isturf(T3) || isspaceturf(T3))
				continue
			if(locate(/obj/structure/dense_ribbon)/*|| locate(/turf/closed/indestructible/reinforced)*/ in T3)
				continue
...
/mob/living/simple_animal/hostile/abnormality/pink_shoes/proc/RibbonVisual(mob/living/simple_animal/hostile/user)//TODO: make it copy pixel_x and pixel_y offsets
...
/mob/living/simple_animal/hostile/abnormality/pink_shoes/proc/Convert(mob/living/target)//same thing as the status effect
...
/datum/status_effect/display/pinkshoes//doesn't do anything, just visual flair
	id = "pinkshoes"
...
/obj/structure/spreading/pink_ribbon
	...
	max_integrity = 45//they're a bit harder to break
	armor = list(
		MELEE = 0,
		BULLET = 0,
		FIRE = -50,
...
/obj/effect/root/pinkshoes/proc/explode() //repurposed code from artillary bees, a delayed attack

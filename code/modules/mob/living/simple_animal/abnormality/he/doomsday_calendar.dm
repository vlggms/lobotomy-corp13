...
/mob/living/simple_animal/hostile/abnormality/doomsday_calendar/death()
	...
	for(var/mob/living/simple_animal/hostile/abnominion/doomsday_doll/D in spawned_dolls) //delete the dolls when suppressed
		D.death()
...
/mob/living/simple_animal/hostile/abnormality/doomsday_calendar/user_buckle_mob(mob/living/M, mob/user, check_loc)
	...
	if(do_after(user, 20, target = M))
		if(!ishuman(M) && !istype(M, /mob/living/simple_animal/hostile/abnominion/doomsday_doll))
			to_chat(user, span_warning("[src] rejects your offering!"))
			return
		if(istype(M ,/mob/living/simple_animal/hostile/abnominion/doomsday_doll))
			spawned_dolls -= M
...
//***Simple Mobs***//
//clay dolls
/mob/living/simple_animal/hostile/abnominion/doomsday_doll
...
/mob/living/simple_animal/hostile/abnominion/doomsday_doll/Initialize()
	. = ..()
...
/mob/living/simple_animal/hostile/abnormality/doomsday_calendar/proc/SpawnAdds()
	...
	for(var/i = doll_count, i < doll_count_maximum, i++)//This counts up
		var /mob/living/simple_animal/hostile/abnominion/doomsday_doll/D= new(get_turf(src))
		spawned_dolls += D

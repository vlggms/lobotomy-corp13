...
/mob/living/simple_animal/hostile/abnormality/kqe/Life()
	...
	if(!heart)
		...
		if(!LAZYLEN(GLOB.department_centers))
			...
			var/mob/living/simple_animal/hostile/abnominion/kqe_heart/H = new(get_turf(X))
			heart = H
			H.abno_host = src
		Stagger()
...
/mob/living/simple_animal/hostile/abnormality/kqe/death()
	...
	if(istype(heart, /mob/living/simple_animal/hostile/abnominion/kqe_heart))
		qdel(heart)
	..()
...
/mob/living/simple_animal/hostile/abnominion/kqe_heart
...
/mob/living/simple_animal/hostile/abnominion/kqe_heart/Move()
	return FALSE

/mob/living/simple_animal/hostile/abnominion/kqe_heart/CanAttack(atom/the_target)//should only attack when it has fists
	return FALSE

/mob/living/simple_animal/hostile/abnominion/kqe_heart/death()
	if(abno_host)
		abno_host.death()
	animate(src, alpha = 0, time = 10 SECONDS)
	QDEL_IN(src, 10 SECONDS)

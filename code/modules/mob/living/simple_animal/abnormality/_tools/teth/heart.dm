...
/datum/status_effect/display/aspiration/tick()
	...
	if(ferventbeats && rage_safe_time < world.time)
		H.deal_damage(H.maxHealth * (1/100), BRUTE, attack_type = (ATTACK_TYPE_STATUS)) // Roughly standard regenerator healing
	if(!raging)
		return
	...
	H.deal_damage(H.maxHealth * (2/100), BRUTE, attack_type = (ATTACK_TYPE_STATUS)) // You are most likely going to die, and very soon.

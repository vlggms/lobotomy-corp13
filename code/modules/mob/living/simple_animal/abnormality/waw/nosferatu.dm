...
/mob/living/simple_animal/hostile/abnormality/nosferatu/PostWorkEffect(mob/living/carbon/human/user, work_type, pe, work_time)
	if(work_type == ABNORMALITY_WORK_REPRESSION) // Owe we got repessed
		AdjustThirst(-250)
		return
...
/mob/living/simple_animal/hostile/abnormality/nosferatu/update_icon_state()
	...
	if(berzerk || incorporeal_move) // Big angry bat form
...
/mob/living/simple_animal/hostile/abnormality/nosferatu/proc/Berzerk()
	if(incorporeal_move || IsContained()) // No bricking the mob by Berzerking when we aren't supposed to.
		return
	AdjustThirst(-3000)
...
/mob/living/simple_animal/hostile/abnormality/nosferatu/proc/Banquet()//AOE attack
	if(incorporeal_move) // No attack abilities while in mist form
		return
	banquet_cooldown = world.time + banquet_cooldown_time
	can_act = FALSE
...
/mob/living/simple_animal/hostile/abnormality/nosferatu/handle_automated_action()
	...
	if(!incorporeal_move && (mist_cooldown <= world.time))
		FleeNow()

/mob/living/simple_animal/hostile/abnormality/nosferatu/proc/FleeNow() // Literally big wolf ripped code, should actually perform ok
	if(!can_act)
		return
	mist_cooldown = world.time + mist_cooldown_time
	MistForm()

...
/mob/living/simple_animal/hostile/abnormality/general_b/Initialize()
	...
	if(IsCombatMap())
		...
		if(SSmaptype.maptype == "limbus_labs")
			var/mob/living/simple_animal/hostile/abnominion/soldier_bee/V = new(get_turf(src))
			beespawn+=V
			V = new(get_turf(src))
			beespawn+=V
	else
...
/mob/living/simple_animal/hostile/abnormality/general_b/ZeroQliphoth(mob/living/carbon/human/user)
	...
	for(var/mob/living/simple_animal/hostile/abnominion/artillery_bee/artillerbee in GLOB.mob_living_list)
		artillerbee_count++
...
/mob/living/simple_animal/hostile/abnormality/general_b/proc/SpawnMinions(list/spawn_turfs, copy_faction = FALSE)
	...
	for(var/turf/Y in spawn_turfs)
		if(prob(60))
			spawn_minion = new /mob/living/simple_animal/hostile/abnominion/soldier_bee(Y)
		else if(prob(20))
			spawn_minion = new /mob/living/simple_animal/hostile/abnominion/artillery_bee(Y)
...
/mob/living/simple_animal/hostile/abnormality/general_b/proc/SpawnBees()
	...
	new /mob/living/simple_animal/hostile/abnominion/artillery_bee(T)
	for(var/y = 1 to 5)
		new /mob/living/simple_animal/hostile/abnominion/soldier_bee(T)

/* Soldier bees */
/mob/living/simple_animal/hostile/abnominion/soldier_bee
	name = "soldier bee"
...
/* Artillery bees */
/mob/living/simple_animal/hostile/abnominion/artillery_bee
	name = "artillery bee"
...
/mob/living/simple_animal/hostile/abnominion/artillery_bee/Login()
	. = ..()
	if(!. || !client)
		return FALSE
	sight_ability.original_sight = src.sight
...
/mob/living/simple_animal/hostile/abnominion/artillery_bee/Initialize()
	. = ..()
...
/mob/living/simple_animal/hostile/abnominion/artillery_bee/Life()
	. = ..()
	if(!.) // Dead
		return FALSE
	if ((!combat_map) && (!client))
...
/mob/living/simple_animal/hostile/abnominion/artillery_bee/proc/AimShell()
	fire_cooldown = world.time + fire_cooldown_time
	var/list/targets = list()
	for(var/mob/living/L in livinginrange(fireball_range, src))
		if(L.z != z)
...
/mob/living/simple_animal/hostile/abnominion/artillery_bee/proc/FireShell(target, called_by_ability)
	var/turf/target_turf = get_turf(target)
	if(target_turf.density)
		to_chat(src, span_notice("Can't fire at that location!"))
		if(called_by_ability) // Used so that Perform() can return on the targeted ability.

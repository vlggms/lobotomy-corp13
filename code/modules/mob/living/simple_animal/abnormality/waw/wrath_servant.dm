...
/mob/living/simple_animal/hostile/abnormality/wrath_servant/Life()
	...
	if(stunned && COOLDOWN_FINISHED(src, stun))
		for(var/mob/living/L in urange(10, src))
			if(L.z != z)
				continue
			if(istype(L, /mob/living/simple_animal/hostile/abnominion/azure_hermit) || istype(L, /mob/living/simple_animal/hostile/abnominion/azure_stave))
				continue
	...
	if(stunned)
		return
	var/mob/living/simple_animal/hostile/abnominion/azure_hermit/AH = locate() in view(5, src)
...
/mob/living/simple_animal/hostile/abnormality/wrath_servant/Found(atom/A)
	if(istype(A, /mob/living/simple_animal/hostile/abnormality/nihil)) // 1st Priority
		return TRUE
	if(istype(A, /mob/living/simple_animal/hostile/abnominion/azure_stave)) // 2nd Priority
		return TRUE
	if(istype(A, /mob/living/simple_animal/hostile/abnominion/azure_hermit)) // 3rd Priority
		return TRUE
...
/mob/living/simple_animal/hostile/abnormality/wrath_servant/AttackingTarget(atom/attacked_target)
	...
	if(!istype(attacked_target, /mob/living/simple_animal/hostile/abnominion/azure_hermit))
		return
	var/mob/living/simple_animal/hostile/abnominion/azure_hermit/AZ = attacked_target
	if(AZ.health > 40)
		return
	PerformEnding(AZ)
...
/mob/living/simple_animal/hostile/abnormality/wrath_servant/BreachEffect(mob/living/carbon/human/user, breach_type)
	...
	for(var/turf/dep in GLOB.department_centers)
		...
		if(get_dist(src, dep) < get_dist(src, target_turf))
			continue
		target_turf = dep
	new /mob/living/simple_animal/hostile/abnominion/azure_hermit(target_turf)
...
/mob/living/simple_animal/hostile/abnormality/wrath_servant/proc/PerformEnding(mob/living/simple_animal/hostile/abnominion/azure_hermit/target = null)
	ending = TRUE
	can_act = FALSE
	target.gib(TRUE)
	adjustBruteLoss(-maxHealth, forced = TRUE)
...
/mob/living/simple_animal/hostile/abnormality/wrath_servant/proc/NihilModeEnable()
	...
	for(var/mob/living/simple_animal/hostile/abnominion/azure_hermit/badguy in world)
		badguy.gib(TRUE)
...
//Rival's code
/mob/living/simple_animal/hostile/abnominion/azure_hermit
...
/mob/living/simple_animal/hostile/abnominion/azure_hermit/Initialize()
	. = ..()
	COOLDOWN_START(src, conjure, conjure_cooldown)
	animate(src, 10, alpha = 255)

/mob/living/simple_animal/hostile/abnominion/azure_hermit/Found(atom/A)
...
/mob/living/simple_animal/hostile/abnominion/azure_hermit/Life()
	. = ..()
...
/mob/living/simple_animal/hostile/abnominion/azure_hermit/OpenFire(atom/A)
	if(!can_act)
		return
...
/mob/living/simple_animal/hostile/abnominion/azure_hermit/AttackingTarget(atom/attacked_target)
	if(!can_act || (status_flags & GODMODE))
		return
	...
	if(ishuman(attacked_target))
		...
		if(get_user_level(H) < 3)
			...
			var/mob/living/simple_animal/hostile/abnominion/azure_stave/AS = new(TT)
			staves += AS
			AS = new(TT)
			staves += AS
			return
...
/mob/living/simple_animal/hostile/abnominion/azure_hermit/Move(atom/newloc, dir, step_x, step_y)
	if(!can_act)
		return
	if(status_flags & GODMODE)
		return
...
/mob/living/simple_animal/hostile/abnominion/azure_hermit/proc/Conjure()
	...
	for(var/i = 0 to min(rand(1, 3), valid_turfs.len))
		var/mob/living/simple_animal/hostile/abnominion/azure_stave/AS = new(pick(valid_turfs))
		staves += AS
	COOLDOWN_START(src, conjure, conjure_cooldown)
	return

/mob/living/simple_animal/hostile/abnominion/azure_hermit/proc/Befuddle()
	if(!can_act || (status_flags & GODMODE))
		return
	can_act = FALSE
...
/mob/living/simple_animal/hostile/abnominion/azure_hermit/proc/Downed()
...
/mob/living/simple_animal/hostile/abnominion/azure_hermit/death()
	INVOKE_ASYNC(src, PROC_REF(Downed))

/mob/living/simple_animal/hostile/abnominion/azure_hermit/gib(real = FALSE)
	if(!real)
		death()
		return
	can_act = FALSE
...
/mob/living/simple_animal/hostile/abnominion/azure_stave
	name = "Hermit's Staff"
...
/datum/status_effect/wrath_burning/tick()
	...
	if((status_holder.sanityhealth <= 0) || (status_holder.health <= 0))
		...
		new /mob/living/simple_animal/hostile/abnominion/azure_stave(spawner_turf)

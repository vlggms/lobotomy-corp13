...
/mob/living/simple_animal/hostile/abnormality/thunder_bird
	...
	var/list/thunder_bird_lines = list(
				...
				"Think I am happy, feather? Think again!",
	...
	speed = 2
	move_to_delay = 3
	damage_coeff = list(BRUTE = 1, RED_DAMAGE = 1.2, WHITE_DAMAGE = 0.5, BLACK_DAMAGE = 1.5, PALE_DAMAGE = 1)
...
/*---Combat---*/
	//range and attack speed for thunder bombs
	var/fire_cooldown_time = 3 SECONDS
	var/fireball_range = 5
	var/fire_cooldown

	//Melee stats
	attack_sound = 'sound/abnormalities/thunderbird/Warp_Blow_Stab.wav'
...
/mob/living/simple_animal/hostile/abnormality/thunder_bird/proc/thunder_bird_dash(target)
	...
	playsound(src, 'sound/abnormalities/thunderbird/WCorp_Special.wav', 100, 1)
...
/mob/living/simple_animal/hostile/abnormality/thunder_bird/proc/do_dash(move_dir, times_ran)
	...
	for(var/obj/machinery/door/D in T.contents)
		if(D.density)
			D.open(2)
	if(stop_charge)
		playsound(src, 'sound/abnormalities/thunderbird/Abiter_ThinLine.wav', 75, 1)
		charging = FALSE
		icon_state = "thunderbird_breach"
		return
	forceMove(T)
	playsound(src,"sound/abnormalities/thunderbird/Warp_Blow_Stab.wav", rand(50, 70), 1)
...
/mob/living/simple_animal/hostile/abnormality/thunder_bird/SuccessEffect(mob/living/carbon/human/user, work_type, pe)
	if(user.health > (user.maxHealth*0.8))
		...
		playsound(src, 'sound/abnormalities/thunderbird/Abiter_ThinLine.wav', 50, TRUE)
...
/mob/living/simple_animal/hostile/thunder_zombie
	...
	attack_sound = 'sound/abnormalities/thunderbird/Warp_Mass_Vert.wav'

	/*Zombie Stats */
	health = 200//subject to change
	maxHealth = 200
	obj_damage = 300
...
/mob/living/simple_animal/hostile/thunder_zombie/proc/Convert(mob/living/carbon/human/H)
	...
	playsound(src, 'sound/abnormalities/thunderbird/Tomary_Phase2.wav', 45, FALSE, 5)
	SLEEP_CHECK_DEATH(3)
	for(var/i = 1 to 4)
...
/mob/living/simple_animal/hostile/thunder_zombie/Initialize()
	..()
	GLOB.zombies += src
	playsound(get_turf(src), 'sound/abnormalities/thunderbird/WCorp_Special.wav', 50, 1, 4)
...
/mob/living/simple_animal/hostile/abnormality/thunder_bird/proc/fireshell()
	...
	for(var/mob/living/carbon/human/L in livinginrange(fireball_range, src))
		if(faction_check_mob(L, FALSE))
			continue
		targets += L
	new /obj/effect/thunderbolt(get_turf(pick(targets)))
...
/obj/effect/thunderbolt/proc/explode()
	playsound(get_turf(src), 'sound/abnormalities/thunderbird/Abiter_ThinLine.wav', 50, 0, 8)

...
/mob/living/simple_animal/hostile/abnormality/sirocco/proc/Grabber()
	for(var/mob/living/L in livinginview(2, src)) // Grab anyone in our range
		if(faction_check_mob(L))
			continue
		L.throw_at(src, 1, 1, src, FALSE)
...
/mob/living/simple_animal/hostile/abnormality/sirocco/proc/ThrowAround() // Make this do stuff
	...
	for(var/mob/living/L in turf_underneath)
		if(faction_check_mob(L))
			continue
		var/randomdir = rand(0, 8)
		L.throw_at(get_edge_target_turf(src,randomdir), 5, 3, src, TRUE)

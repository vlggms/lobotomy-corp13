...
/obj/item/ego_weapon/mini/limos
	...
	icon_state = "limos"
	icon = 'code/modules/mob/living/simple_animal/abnormality/_auxiliary_modes/community/!icons/ego_weapons.dmi'
	force = 20 // VERY fast attacks potentially
...
/obj/item/ego_weapon/lance/miasma/UserMoved(mob/user)
	...
	if(prob(25))
		var/grab_dir = turn(user.dir, pick(-90, 90, 180, 180)) //grab a random direction away from where the user is facing
		var/turf/spawn_turf = get_ranged_target_turf(user, grab_dir, rand(1, 3))
		if(!spawn_turf)
			return
		var/turf/proj_turf = user.loc
		if(!isturf(proj_turf))
			return
		var/obj/projectile/ego_bullet/miasma/G = new /obj/projectile/ego_bullet/miasma(proj_turf)
		G.fired_from = src //for signal check
		playsound(user, 'sound/effects/venom.ogg', 50, TRUE)
		G.firer = user
		G.preparePixelProjectile(spawn_turf, user, null)
		G.fire()
		G.damage*=force_multiplier
		var/userjust = (get_modified_attribute_level(user, JUSTICE_ATTRIBUTE))
		var/justicemod = 1 + userjust/100
...
/obj/projectile/ego_bullet/miasma
	name = "toxic glob"
...
/obj/projectile/ego_bullet/miasma/Impact(atom/A)
	new /obj/effect/gibspawner/generic/silent/liquid_miasma(get_turf(src))
	return ..()

/obj/projectile/ego_bullet/miasma/on_hit(atom/target, blocked = FALSE)
	if(!ishuman(target))
		nodamage = FALSE
	else
		return

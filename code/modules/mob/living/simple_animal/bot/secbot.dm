...
/mob/living/simple_animal/bot/secbot/proc/retaliate(mob/living/carbon/human/H)
	...
	threatlevel = H.assess_threat_tegu(judgement_criteria, src, TRUE, null, weaponcheck=CALLBACK(src, .proc/check_for_weapons)) // TEGUSTATION SECBOT FEEDBACK PR -Surrealistik Feb 2020
	threatlevel += 6
	if(threatlevel >= 4)
		target = H
		mode = BOT_HUNT
...
/mob/living/simple_animal/bot/secbot/proc/stun_attack(mob/living/carbon/C, harm = FALSE)
	...
	if(ishuman(C))
		...
		threat = H.assess_threat_tegu(judgement_criteria, src, FALSE, null, weaponcheck=CALLBACK(src, .proc/check_for_weapons)) // TEGUSTATION SECBOT FEEDBACK PR -Surrealistik Feb 2020
	else
		C.Paralyze(100)
		C.stuttering = 5
	...
	if(declare_arrests)
		secbot_declare_arrest_completion(C, threat) // TEGUSTATION IMPROVED RECORD SECURITY PR -Surrealistik Oct 2019; this makes a record of the arrest, including timestamp and location.
		//var/area/location = get_area(src) // TEGU REMOVE
		//speak("[arrest_type ? "Detaining" : "Arresting"] level [threat] scumbag <b>[C]</b> in [location].", radio_channel)  // TEGU REMOVE
...
/mob/living/simple_animal/bot/secbot/proc/look_for_perp()
	...
	for (var/mob/living/carbon/C in view(7,src)) //Let's find us a criminal
		...
		tegu_threat_assess_carbon_filter(C, judgement_criteria) // TEGUSTATION SECBOT FEEDBACK PR -Surrealistik Feb 2020
		...
		if(!threatlevel)
			...
			if(declare_arrests) // TEGUSTATION IMPROVED RECORD SECURITY PR -Surrealistik Jan 2020
				secbot_declare_arrest_attempt(C, threatlevel) // TEGUSTATION IMPROVED RECORD SECURITY PR -Surrealistik Jan 2020
...
/mob/living/simple_animal/bot/secbot/proc/check_for_weapons(obj/item/slot_item)
	if(slot_item && (slot_item.item_flags & NEEDS_PERMIT))
		return slot_item // TEGUSTATION SECBOT FEEDBACK PR -Surrealistik Feb 2020
	return FALSE

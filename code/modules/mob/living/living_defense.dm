...
/mob/living/bullet_act(obj/projectile/P, def_zone, piercing_hit = FALSE)
	...
	if(!P.nodamage && on_hit_state != BULLET_ACT_BLOCK)
		deal_damage(P.damage * P.damage_multiplier, P.damage_type, source = P.firer, flags = P.white_healing ? (DAMAGE_WHITE_HEALABLE) : null, attack_type = (ATTACK_TYPE_RANGED), def_zone = def_zone, wound_bonus=P.wound_bonus, bare_wound_bonus=P.bare_wound_bonus, sharpness = P.sharpness)
...
/// Called by the deal_damage() wrapper.. Return TRUE to receive the damage, return FALSE to prevent damage from being taken.

...
/datum/preferences/proc/SetBackstory(mob/user)
	if(!SSbackstories)
		to_chat(user, span_danger("The backstories subsystem is still initializing! Try again in a minute."))
		return

	var/list/dat = list()
	if(!SSbackstories.backstories.len)
		dat += "The backstories subsystem hasn't finished initializing, please hold..."
		...
		for(var/B in SSbackstories.backstories)
			var/datum/backstory/T = SSbackstories.backstories[B]
			var/backstory_name = initial(T.name)
			var/font_color = "#AAAAFF"
			dat += "<a href='?_src_=prefs;preference=trait;task=update;trait=[backstory_name]'>Take</a> \
			<font color='[font_color]'>[backstory_name]</font> - [initial(T.desc)]<br>"
		dat += "<br><center><a href='?_src_=prefs;preference=trait;task=reset'>Reset Backstories</a></center>"
	var/datum/browser/popup = new(user, "mob_occupation", "<div align='center'>Character Backstory</div>", 900, 600) //no reason not to reuse the occupation window, as it's cleaner that way
	popup.set_window_options("can_close=0")
	popup.set_content(dat.Join())
	popup.open(FALSE)
	...
	if(href_list["preference"] == "job")
		...
		switch(href_list["task"])
			if("close")
				...
				ShowChoices(user)
			if("update")
				var/new_backstory = href_list["trait"]
				if(!SSbackstories.backstories[new_backstory])
					return

...
/proc/show_blurb(client/C, duration, blurb_text, fade_out_time = 5, fade_in_time = 10, text_color = "white", outline_color = "black", text_align = "center", screen_location = "LEFT+1,BOTTOM+2", marquee = FALSE, outline_width = 1)
	if(!C)
		return

	var/style = "font-family: 'Better VCR'; text-align: [text_align]; color: [text_color]; -dm-text-outline: [outline_width] [outline_color]; font-size: 11px;"
	var/text = blurb_text
	text = uppertext(text)

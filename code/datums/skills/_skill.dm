...
/datum/skill/proc/try_skill_reward(datum/mind/mind, new_level)
	...
	if (!ispath(skill_cape_path))
		to_chat(mind.current, "<span class='nicegreen'>My legendary [name] skill is quite impressive, though it seems the Professional [title] Association doesn't have any status symbols to commemorate my abilities with. I should let HQ know of this travesty, maybe they can do something about it.</span>")
		return

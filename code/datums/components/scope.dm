...
//	RegisterSignal(parent, COMSIG_ITEM_AFTERATTACK_SECONDARY, PROC_REF(on_secondary_afterattack)) LOBOTOMYCORPORATION EDIT OLD
	RegisterSignal(parent, COMSIG_ITEM_MIDDLE_CLICK_ACTION, PROC_REF(on_secondary_afterattack)) // LOBOTOMYCORPORATION EDIT NEW
...
//		COMSIG_ITEM_AFTERATTACK_SECONDARY, LOBOTOMYCORPORATION EDIT OLD
		COMSIG_ITEM_MIDDLE_CLICK_ACTION, // LOBOTOMYCORPORATION EDIT NEW
		COMSIG_GUN_TRY_FIRE,
...
/datum/component/scope/proc/on_secondary_afterattack(datum/source, atom/target, mob/user, proximity_flag, click_parameters)
	...
	if(tracker)
		...
		start_zooming(user)
	return COMPONENT_CANCEL_MIDDLE_CLICK_ACTION
...
/datum/component/scope/proc/on_gun_fire(obj/item/gun/source, mob/living/user, atom/target, flag, params)
	...
	if(!tracker?.given_turf || target == get_target(tracker.given_turf))
		return NONE
//	INVOKE_ASYNC(source, TYPE_PROC_REF(/obj/item/gun, fire_gun), get_target(tracker.given_turf), user) LOBOTOMYCORPORATION EDIT OLD
	INVOKE_ASYNC(source, TYPE_PROC_REF(/obj/item/gun, afterattack), get_target(tracker.given_turf), user) // LOBOTOMYCORPORATION EDIT NEW
	return COMPONENT_CANCEL_GUN_FIRE

/datum/component/scope/proc/on_examine(datum/source, mob/user, list/examine_list)
	SIGNAL_HANDLER

//	examine_list += span_notice("You can scope in with <b>right-click</b>.") LOBOTOMYCORPORATION EDIT OLD
	examine_list += span_notice("You can scope in with <b>middle-click</b>.") // LOBOTOMYCORPORATION EDIT NEW

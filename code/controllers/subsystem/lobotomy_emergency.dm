...
/datum/controller/subsystem/lobotomy_emergency/proc/SetEmergencyLevel(level, forced = FALSE)
	...
	switch(level)
		if(TRUMPET_0)
			custom_minor_announce(CONFIG_GET(string/alert_warning_reset), "Attention!",'sound/misc/notice2.ogg')
			...
			for(var/obj/machinery/firealarm/FA in GLOB.machines)
				if(is_station_level(FA.z))
					FA.update_icon()
		if(TRUMPET_1)
			custom_minor_announce(CONFIG_GET(string/alert_first_trumpet), "Attention! First Trumpet!",'sound/misc/siren.ogg', 80)
			...
			for(var/obj/machinery/firealarm/FA in GLOB.machines)
				if(is_station_level(FA.z))
					FA.update_icon()
		if(TRUMPET_2)
			custom_minor_announce(CONFIG_GET(string/alert_second_trumpet), "Attention! Second Trumpet!",'sound/effects/alertbeep.ogg', 70)
			if(SSshuttle.emergency.mode == SHUTTLE_CALL || SSshuttle.emergency.mode == SHUTTLE_RECALL)
				if(GLOB.emergency_level == TRUMPET_1)
			...
			for(var/obj/machinery/computer/shuttle/pod/pod in GLOB.machines)
				pod.locked = FALSE
		if(TRUMPET_3)
			custom_minor_announce(CONFIG_GET(string/alert_third_trumpet), "Attention! Third Trumpet!")
			if(SSshuttle.emergency.mode == SHUTTLE_CALL || SSshuttle.emergency.mode == SHUTTLE_RECALL)

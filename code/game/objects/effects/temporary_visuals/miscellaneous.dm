...
/obj/effect/temp_visual/dir_setting/longbloodsplatter
	icon = 'icons/effects/blood.dmi'
	duration = 5
	randomdir = FALSE
	layer = BELOW_MOB_LAYER
	var/splatter_type = "splatter_white"

/obj/effect/temp_visual/dir_setting/longbloodsplatter/Initialize(mapload, set_dir, offsetx, offsety)
	if(ISDIAGONALDIR(set_dir))
		icon_state = "[splatter_type][pick(1, 2, 6)]"
	else
		icon_state = "[splatter_type][pick(3, 4, 5)]"
	...
	switch(set_dir)
		...
		if(WEST)
			target_pixel_x = -32
		if(NORTHEAST)
			target_pixel_x = 22.627
			target_pixel_y = 22.627
		if(NORTHWEST)
			target_pixel_x = -22.627
			target_pixel_y = 22.627
		if(SOUTHEAST)
		...
		if(SOUTHWEST)
			...
			target_pixel_y = -22.627
			layer = ABOVE_MOB_LAYER
	animate(src, pixel_x = pixel_x + target_pixel_x, pixel_y = pixel_y + target_pixel_y, alpha = 0, time = duration)

/obj/effect/temp_visual/dir_setting/longbloodsplatter/purple
	color = "#783894"

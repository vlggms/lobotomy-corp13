...
//	if(!isliving(user) || !in_range(src, user) || user.combat_mode) LOBOTOMYCORPORATION EDIT CHANGE OLD
	if(!isliving(user) || !in_range(src, user) || user.a_intent == INTENT_HARM) // LOBOTOMYCORPORATION EDIT CHANGE NEW - we dont use combat mode
		return

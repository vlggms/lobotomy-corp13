...
// TODO: Mirage Storm attack effects, possibly (Lacerate each enemy and apply defense buff at the end maybe?)
/obj/item/ego_weapon/city/pt //This shouldn't be obtainable, like at all
	...
	actions_types = list(/datum/action/item_action/charging/miragestorm)
...
/obj/item/ego_weapon/city/pt/attack(mob/living/target, mob/living/user)
	if(!CanUseEgo(user))
		return
	..()
	for(var/datum/action/item_action/charging/miragestorm/CA in actions)
		CA.charge += 1
	if(boost)
		boost = FALSE
		force = force/1.5

/obj/item/ego_weapon/city/pt/ui_action_click(mob/living/user, actiontype) //Stolen from Blue Sicko
	if(istype(actiontype, /datum/action/item_action/charging/miragestorm))
		MirageStorm(user)
	return

/obj/item/ego_weapon/city/pt/ComponentInitialize()
	. = ..()
	AddElement(/datum/element/update_icon_updates_onmob)

/obj/item/ego_weapon/city/pt/item_action_slot_check(slot, mob/living/user)
...
// TODO: Store starting stance and apply weapon after mirage storm
// make it so you can't attack while mirage storming
//
/obj/item/ego_weapon/city/pt/proc/MirageStorm(mob/living/user)
	...
	if(!LAZYLEN(targets))
		...
		var/datum/action/item_action/charging/miragestorm/T = locate() in actions
		T.AddCharge(T.max_charge) // Refund
		active = FALSE
		return

	ADD_TRAIT(src, TRAIT_NODROP, STICKY_NODROP) //Dropping the weapons kinda cringe
	...
	icon_state = "ptslash" //bandaid solution for now, till i figure out how to change the weapon in hand
	inhand_icon_state = "ptslash"
	force = 45
	damtype = RED_DAMAGE
	armortype = RED_DAMAGE
	...
	for(var/mob/living/L in targets) //the fucking 5TH FOR LOOP, AHHHHHH.
		L.apply_status_effect(/datum/status_effect/pt_lacerate)
	targets = list()
...
/obj/item/ego_weapon/city/pt/proc/MirageAttack(mob/living/user)
	for(var/mob/living/L in targets)
		...
		src.attack(L, user)
		prev_loc.Beam(tp_loc, "plasmabeam", time=10)
...
/datum/action/item_action/charging/miragestorm
	name = "Mirage Storm"

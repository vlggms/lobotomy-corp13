...
/obj/item/dyespray/proc/dye(mob/target)
	...
	var/new_grad_color = input(usr, "Choose a secondary hair color:", "Character Preference","#"+human_target.gradient_color) as color|null
	if(!new_grad_color)
		return

	human_target.gradient_style = new_grad_style
	human_target.gradient_color = sanitize_hexcolor(new_grad_color)

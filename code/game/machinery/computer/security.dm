...
//RECORD DELETE
			if("Delete All Records")
				temp = ""
				temp = delete_allrecords_feedback() // TEGUSTATION IMPROVED RECORD SECURITY PR -Surrealistik Oct 2019
				/*
			...
			if("Delete Record (ALL)")
				if(active1)
					delete_record_feedback("ALL") // TEGUSTATION IMPROVED RECORD SECURITY PR -Surrealistik Oct 2019
					/*
			...
			if("Delete Record (Security)")
				if(active2)
					delete_record_feedback() // TEGUSTATION IMPROVED RECORD SECURITY PR -Surrealistik Oct 2019
					/*
...
//FIELD FUNCTIONS
			if("Edit Field")
				...
				switch(href_list["field"])
					if("name")
						if(istype(active1, /datum/data/record) || istype(active2, /datum/data/record))
							if(!check_input_clearance(usr)) // TEGUSTATION IMPROVED RECORD SECURITY PR -Surrealistik Oct 2019

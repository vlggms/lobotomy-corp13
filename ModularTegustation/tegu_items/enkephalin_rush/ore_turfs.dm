...
/turf/closed/mineral/random/facility/proc/AbnoEvent(user)
	...
	if(abno_level > 5)//don't go out of bounds!
		abno_level = 5
	...
	if(abno)
		...
		addtimer(CALLBACK(A, TYPE_PROC_REF(/mob/living/simple_animal/hostile/abnormality, BreachEffect), user, BREACH_MINING), 2 SECONDS)//In a separate PR: Add BREACH_MINING. Use this to prevent abnos from teleporting and to nerf pink midnights

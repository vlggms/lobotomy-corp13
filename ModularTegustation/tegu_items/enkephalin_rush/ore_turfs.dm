...
/turf/closed/mineral/random/facility/Initialize()
	if(prob(30) && (baseturfs == /turf/open/floor/plating))
...
/turf/closed/mineral/random/facility/proc/AbnoEvent(user)
	visible_message(span_danger("spawning abnormality!"))
	...
	if(abno)
		...
		//addtimer(CALLBACK(A, TYPE_PROC_REF(/mob/living/simple_animal/hostile/abnormality, PostSpawn)), 1 SECONDS)
		//The above causes issues with specific abnos like amurdad, but fixes it for others like One Sin. Needs a better solution
		addtimer(CALLBACK(A, TYPE_PROC_REF(/mob/living/simple_animal/hostile/abnormality, BreachEffect), user, BREACH_PINK), 2 SECONDS)//In a separate PR: Add BREACH_MINING. Use this to prevent abnos from teleporting and to nerf pink midnights

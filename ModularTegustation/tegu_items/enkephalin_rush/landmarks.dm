...
/obj/effect/landmark/abnospawn/assiyah/Initialize(mapload)//Make better subtypes instead of repeating all this code
	..()
	var/spawning = pick_n_take(GLOB.assiyah)
	if(spawning)
		var/mob/living/simple_animal/hostile/abnormality/A = new spawning(get_turf(src))
		addtimer(CALLBACK(A, TYPE_PROC_REF(/mob/living/simple_animal/hostile/abnormality, BreachEffect), null, BREACH_MINING), 20 SECONDS)
...
/obj/effect/landmark/abnospawn/briah/Initialize(mapload)
	..()
...
/obj/effect/landmark/abnospawn/atziluth/Initialize(mapload)
	..()
	var/spawning = pick_n_take(GLOB.atziluth)
	if(spawning)
		var/mob/living/simple_animal/hostile/abnormality/A = new spawning(get_turf(src))
		addtimer(CALLBACK(A, TYPE_PROC_REF(/mob/living/simple_animal/hostile/abnormality, BreachEffect), null, BREACH_MINING), 60 SECONDS)

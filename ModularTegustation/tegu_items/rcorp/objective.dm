...
/obj/structure/bough/proc/FilterLoop(loop_stage) //Takes a numeric argument for advancing the loop's stage in a cycle (1 > 2 > 3 > 1 > ...)
	if(filters[filters.len]) // Stops the loop if we have no filters to animate
		switch(loop_stage)
			if(1)
				animate(f1, type="ripple", , radius = 60, time = 60, flags = CIRCULAR_EASING | EASE_OUT | ANIMATION_PARALLEL)
				animate(f2, type="rays", , size = 30, offset = pick(4,5,6), time = 60, flags = SINE_EASING | EASE_OUT | ANIMATION_PARALLEL)
				addtimer(CALLBACK(src, .proc/FilterLoop, 2), 6 SECONDS)
			if(2)
				animate(f1, type="ripple", size = 25, radius = 80, time = 20, flags = CIRCULAR_EASING | EASE_OUT | ANIMATION_PARALLEL)
				animate(f2, type="rays" , size = 20, offset = pick(0.2,0.4), time = 60, flags = SINE_EASING | EASE_OUT | ANIMATION_END_NOW | ANIMATION_PARALLEL)
				addtimer(CALLBACK(src, .proc/FilterLoop, 3), 2 SECONDS)
			if(3)
				animate(f1, type="ripple", size = 20, radius = 0, time = 0, flags = CIRCULAR_EASING | EASE_IN | EASE_OUT | ANIMATION_PARALLEL)

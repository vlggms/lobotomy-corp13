...
/obj/effect/landmark/objectivespawn/Initialize()
	...
	switch(GLOB.rcorp_objective)
		if("button")
			...
			addtimer(CALLBACK(src, PROC_REF(reinforce)), 20 MINUTES)
		if("vip")
			new /mob/living/simple_animal/hostile/shrimp_vip(get_turf(src))
		if("arbiter")
			new /obj/structure/bough(get_turf(src))
			addtimer(CALLBACK(src, PROC_REF(arbspawn)), 25 MINUTES)
		if("payload_abno")
			new /mob/payload(get_turf(src), "abno")
/*		if("payload_rcorp")					Currently causes the game to crash, we're gonna disable until it no longer does this.

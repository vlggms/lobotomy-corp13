/obj/machinery/regenerator
	...
	var/rapid = FALSE // Set to heal every process tick instead of every 5
	var/disabled = FALSE
...
/obj/machinery/regenerator/proc/Heal(area/A)
	...
	if(LAZYLEN(people_to_heal))
		...
		if(threat_detected)
			threat_mult *= 0.5
		var/hp_amt = (((regeneration_amount + hp_bonus) * threat_mult) + regen_add) * regen_mult
...
/obj/machinery/regenerator/examine(mob/user)
	...
	. += span_info("[src] restores [(((regeneration_amount + hp_bonus) * threat_mult) + regen_add) * regen_mult] HP and [(((regeneration_amount + sp_bonus) * threat_mult) + regen_add) * regen_mult] SP every [time] seconds.")
...
/obj/machinery/regenerator/proc/RapidHeal(mob/living/user)
	if(user)
		to_chat(user, span_notice("[src] is now calibrated to pulse quickly at low output."))
	reset_timer = short_duration + world.time
